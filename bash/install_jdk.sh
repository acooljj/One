...
fun_jdk8_install (){
  jdk_version=8u181
  jdk_downlaod_url="http://download.oracle.com/otn-pub/java/jdk/8u181-b13/96a7b8442fe848ef90c96a2fad6ed6d1/jdk-8u181-linux-x64.tar.gz?AuthParam=1535614924_20b819890973b15ddf6969280d5bd74b"
  #下载安装包
  if [ -f /tmp/jdk-${jdk_version}-linux-x64.tar.gz ];then :;else wget -O /tmp/jdk-${jdk_version}-linux-x64.tar.gz ${jdk_downlaod_url};fi
  #检查安装目录
  mkdir -pv ${jdk_install_path}
  #执行安装
  tar -zxf /tmp/jdk-${jdk_version}-linux-x64.tar.gz -C ${jdk_install_path}
  #增加环境变量
  grep 'export JAVA_HOME=/usr/java/jdk1.8.0_181' /etc/profile || echo 'export JAVA_HOME=/usr/java/jdk1.8.0_181' >> /etc/profile
